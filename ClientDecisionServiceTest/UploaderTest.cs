...
namespace ClientDecisionServiceTest
{
    ...
    public class UploaderTest
    {
        ...
        public void TestUploaderThreadSafeLocal()
        {
            ...
            var createAction = (Func<int, uint>)((i) => { return (uint)i; });
            ...
            Parallel.For(0, numEvents, new ParallelOptions { MaxDegreeOfParallelism = Environment.ProcessorCount * 2 }, (i) =>
            {
                uploader.Upload(new SingleActionInteraction { Action = createAction(i), Context = JsonConvert.SerializeObject(new TestContext()), Probability = createProbability(i), Key = uniqueKey });
            ...
            foreach (CloudBlobContainer container in completeContainers)
            {
                ...
                foreach (var b in blobs)
                {
                    ...
                    {
                        Assert.AreEqual(createAction(i), (uint)interactions[i].Action.Value);
